...
sex_survey_grouped <-
  ...
  mutate(partner_group = case_when(
    partners == 0 & partners_college == 0 ~ "Abstinent",
    partners == 0 & partners_college > 0 ~ "Sexually active since college",
    partners > 0 & partners_college == 0 ~ "Abstinent in college",
    partners > 0 & partners_college > 0 ~ "Sexually active"
  ))
...
sex_survey_grouped %>%
  filter(partner_group == "Abstinent" & !is.na(religious)) %>%
  count(religious)
```
...
-Data was taken mainly from Facebook users
-Data didn't include enough numerical variables, which limited the types of analysis we could do